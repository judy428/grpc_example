...
int main()
{
  ...
  Aservice.RequestSayHello(&(h1->ctx_),&h1->req_,&h1->responder_,cq_new_ptr.get(),cq_notify_ptr.get(),h1);
  Aservice.RequestSayNiHao(&h2->ctx_,&h2->req_,&h2->stream_,cq_new_ptr.get(),cq_notify_ptr.get(),h2);
  Aservice.RequestSayNiHao2(&h3->ctx_,&h3->responder_,cq_new_ptr.get(),cq_notify_ptr.get(),h3);
  Aservice.RequestSayNiHao3(&(h4->ctx_),&(h4->stream_),cq_new_ptr.get(),cq_notify_ptr.get(),h4);
  ...
  std::thread th1([&] {
    ...
    bool okFlag = false;
    while(cq_notify_ptr->Next((void**)&hcb,&okFlag)){
      //cout<<"hcb->type_:"<<hcb->type_<<endl;
      
      std::cout<<"cq_notify_ptr okflag:"<<okFlag<<std::endl;
      // if(okFlag == 0){
      //   if(hcb != nullptr){
        ...
        
        {
          ...
          if(okFlag){
            ...
            if(ins->newCall == 0){
              ...
              h->status_ = 1;
              h->type_ = 3;
              Aservice.RequestSayNiHao2(&h->ctx_,&h->responder_,cq_new_ptr.get(),cq_notify_ptr.get(),h);
      ...
      case 4:
      {
        ...
        if(ins->readWriteFlag == 0){
          ...
          h->status_ = 1;
          h->type_ = 4;
          Aservice.RequestSayNiHao3(&(h->ctx_),&(h->stream_),cq_new_ptr.get(),cq_notify_ptr.get(),h);
        }
  ...
  std::thread th2([&] {
    ...
    while(cq_new_ptr->Next((void**)&hcb,&okFlag)){
      ...
      std::cout<<"cq_new_ptr okflag:"<<okFlag<<std::endl;
      // if(okFlag == 0){
      //   if(hcb != nullptr){
      //     cout<<"okFlag is false ,delete hcb:type"<<hcb->type_<<endl;
      //     delete hcb;
      ...
      // }
      if(hcb == nullptr){
        cout<<"cq_new_ptr read nullptr"<<endl;
        continue;
      }
      if(hcb->status_ == 2){
        cout<<"cq_new_ptr delete hcb:type"<<hcb->type_<<endl;
        delete hcb;
        continue;
      }
      if(hcb == nullptr)
      {
        cout<<"cq_new_ptr hcb == nullptr"<<endl;
      }
